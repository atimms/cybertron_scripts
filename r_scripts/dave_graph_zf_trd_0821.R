...
setwd('/home/atimms/ngs_data/enu_mapping/dave_zf_trd_0821')

##testing on one
hom_data <- read.csv('dave_zf_sex_0920.mom_het_dad_wt.genic.q50_cov10.no_alt.danRer11_10000kb_1000kb.combined_for_r.txt', sep = '\t')
...
hom_data = read.csv('dave_zf_sex_0920.mom_het_dad_wt.genic.q50_cov10.no_alt.danRer11_10000kb_1000kb.aaf_for_r.txt', sep = '\t')
goodChrOrder <- paste("chr",c(1:25),sep="")
hom_data$chr <- factor(hom_data$chr,levels=goodChrOrder)
...
filename <- dir('/home/atimms/ngs_data/enu_mapping/dave_zf_trd_0821', pattern ="aaf_for_r.txt")
...
filename <- dir('/home/atimms/ngs_data/enu_mapping/dave_zf_trd_0821', pattern ="combined_for_r.txt")
for(i in 1:length(filename)){
  ##read in hom data
  hom_data <- read.csv(filename[i], sep ='\t')
  ##reorder chromosome, if using 'chr'
