...
labelOld
labelNew <- c('Early progenitors', 'Ganglion precursors', 'Developing ganglions', 'AC/HC/GC precursors', 'Late progenitors', 'Mature rods', 'Rod precursors', 'Bipolar/photoreceptor precursors', 'Developing cones', 'Developing bipolars', 'Developing amacrines', 'Mature horizontals', 'Mature cones', 'Mature amacrines', 'Mature bipolars', 'Mature bipolars', 'Mature mullers', 'Horizontal precursors', 'Mature ganglions', 'Developing ganglions')
...
##CoAccessibility
proj1 <- addCoAccessibility(ArchRProj = proj1, reducedDims = "Harmony")
...
proj1 <- addPeak2GeneLinks(ArchRProj = proj1,reducedDims = "Harmony")
...
plotPDF(p, name = "plot_rod_traj_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
##pseudotime compared with RHO inferred and rnaseq expression
p1 <- plotTrajectory(proj1, trajectory = "rodU", colorBy = "GeneScoreMatrix", name = "RHO", continuousSet = "horizonExtra")
p2 <- plotTrajectory(proj1, trajectory = "rodU", colorBy = "GeneIntegrationMatrix", name = "RHO", continuousSet = "blueYellow")
ggAlignPlots(p1[[1]], p2[[1]], type = "h")
plotPDF(p1,p2, name = "rod_traj_RHO_expression.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
##pseudotime compared with SAG inferred and rnaseq expression
p1 <- plotTrajectory(proj1, trajectory = "rodU", colorBy = "GeneScoreMatrix", name = "SAG", continuousSet = "horizonExtra")
p2 <- plotTrajectory(proj1, trajectory = "rodU", colorBy = "GeneIntegrationMatrix", name = "SAG", continuousSet = "blueYellow")
ggAlignPlots(p1[[1]], p2[[1]], type = "h")
plotPDF(p1,p2, name = "rod_traj_SAG_expression.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
...
plotPDF(p1, p2, p3, p4, name = "rod_traj_comparison_heatmaps.pdf", ArchRProj = proj1, addDOC = FALSE, width = 6, height = 8)
...
##refine co-accessabilty data
CoAc <- getCoAccessibility(ArchRProj = proj1, corCutOff = 0.6,
                           resolution = 10000, returnLoops = FALSE)
CoAc_2 <- getCoAccessibility(ArchRProj = proj1, corCutOff = 0.45,
                           resolution = 10000, returnLoops = FALSE)
##resolution doesn't change anything
#CoAc_2 <- getCoAccessibility(ArchRProj = proj1, corCutOff = 0.6,
#                           resolution = 1, returnLoops = FALSE)
##coac data i.e. peak ids, chr, corr
CoAc ##we have data for 326,141 tests 
##peak info in granges format
gr <- metadata(CoAc)[[1]]
##better peak info in granges format
gr2 <- getPeakSet(proj1)

##test getting info
head(CoAc$subjectHits,100)
gr2[52:59]$peakType

##get all peak that are promoters
gr2.promoters <- gr2[gr2$peakType == "Promoter"]
gr2.promoters.lsx4 <- gr2.promoters[gr2.promoters$nearestGene == "RHO"]

##write files
write.table(CoAc, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.co_acc.cor6.txt', row.names = F, sep="\t", quote = FALSE)
write.table(CoAc_2, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.co_acc.cor45.txt', row.names = F, sep="\t", quote = FALSE)

df_gr2 = data.frame(gr2)
write.table(df_gr2, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.peak_info.txt', row.names = T, sep="\t", quote = FALSE)

##peak to gene
p2g <- getPeak2GeneLinks(ArchRProj = proj1, corCutOff = 0.45,
                         resolution = 10000, returnLoops = F)
p2g_2 <- getPeak2GeneLinks(ArchRProj = proj1, corCutOff = 0.6,
                         resolution = 1000, returnLoops = F)
atacidx.info = metadata(p2g)[[1]]
rnaidx.info = metadata(p2g)[[2]]
rnaidx.df = data.frame(rnaidx.info)
##write files
write.table(p2g, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.p2g.cor45.txt', row.names = F, sep="\t", quote = FALSE)
write.table(p2g_2, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.p2g.cor6.txt', row.names = F, sep="\t", quote = FALSE)
write.table(rnaidx.df, '/active/cherry_t/OrgManuscript_SingleCell_Data/human_scATAC/archr_analysis/all/co_acc_analysis_0920/human_all.rnaseq_info.txt', row.names = T, sep="\t", quote = FALSE)
...
p <- pheatmap::pheatmap(mat = as.matrix(cM), color = paletteContinuous("whiteBlue"), 
                        border_color = "black")
p
plotPDF(plotList = p, name = "cluster_sample_heatmap.harmony.res0.4.pdf", 
        ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
##umap using harmony
proj1 <- addUMAP(ArchRProj = proj1, reducedDims = "Harmony", 
                 name = "UMAP", nNeighbors = 30, minDist = 0.5, metric = "cosine", force = TRUE)
...
do.call(cowplot::plot_grid, c(list(ncol = 3),p2))
plotPDF(plotList = p, name = "plot_marker_genes_with_imputation.harmony.res0.4.csv.pdf", 
        ArchRProj = proj1,  addDOC = FALSE, width = 5, height = 5)
##get tracks for marker genes and then print
p <- plotBrowserTrack(ArchRProj = proj1, groupBy = "Clusters", 
                      geneSymbol = markerGenes, upstream = 50000, downstream = 50000)
plotPDF(plotList = p, name = "plot_tracks_for_marker_genes.harmony.res0.4.csv.pdf", 
        ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)

##use lsi for umap etc......

##find cluster using seurat (can use LSI or lsi) -- force is needed is repeating
proj1 <- addClusters(input = proj1, reducedDims = "IterativeLSI",
                     method = "Seurat", name = "Clusters", resolution = 0.4, force = TRUE)
counts_cluster_sample = table(proj1$Clusters, proj1$Sample)
write.csv(counts_cluster_sample, file='counts_cluster_sample.lsi.res0.4.csv')
##graph
cM <- confusionMatrix(paste0(proj1$Clusters), paste0(proj1$Sample))
cM <- cM / Matrix::rowSums(cM)
p <- pheatmap::pheatmap(mat = as.matrix(cM), color = paletteContinuous("whiteBlue"), 
                        border_color = "black")
p
plotPDF(plotList = p, name = "cluster_sample_heatmap.lsi.res0.4.pdf", 
        ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
##umap using lsi
...
p2 <- plotEmbedding(ArchRProj = proj1, colorBy = "cellColData", name = "Clusters", embedding = "UMAP")
ggAlignPlots(p1, p2, type = "h")
plotPDF(p1,p2, name = "UMAP_sample_clusters.lsi.res0.4.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
##finding marker genes
markersGS <- getMarkerFeatures(ArchRProj = proj1, useMatrix = "GeneScoreMatrix", 
                               groupBy = "Clusters", bias = c("TSSEnrichment", "log10(nFrags)"),
                               testMethod = "wilcoxon")
markerList <- getMarkers(markersGS, cutOff = "FDR <= 0.01 & Log2FC >= 1.25")
markerList$C6
write.csv(markerList, file='marker_list.lsi.res0.4.csv')
...
do.call(cowplot::plot_grid, c(list(ncol = 3),p2))
plotPDF(plotList = p, name = "plot_marker_genes_with_imputation.lsi.res0.4.csv.pdf", 
        ArchRProj = proj1,  addDOC = FALSE, width = 5, height = 5)
##get tracks for marker genes and then print
p <- plotBrowserTrack(ArchRProj = proj1, groupBy = "Clusters", 
                      geneSymbol = markerGenes, upstream = 50000, downstream = 50000)
plotPDF(plotList = p, name = "plot_tracks_for_marker_genes.lsi.res0.4.pdf", 
        ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)

##trajectory analysis to make sure 
##analysis 090720
trajectory <- c("C19", "C20", "C8", "C2", "C3")
trajectory
proj1 <- addTrajectory(ArchRProj = proj1, name = "trajectory1", 
                       groupBy = "Clusters",trajectory = trajectory, 
                       embedding = "UMAP", force = TRUE)
##each cell has a pseudotime value between 0 and 100
head(proj1$trajectory1[!is.na(proj1$trajectory1)])
p <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "cellColData", name = "trajectory1")
plotPDF(p, name = "trajectory1_traj_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)

##pseudotime compared with inferred expression
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "FGF19", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_FGF19_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "RHO", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_RHO_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "CCND1", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_CCND1_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "PRDM1", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_PRDM1_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "ISL2", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_ISL2_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
p1 <- plotTrajectory(proj1, trajectory = "trajectory1", colorBy = "GeneScoreMatrix", name = "NR2E3", continuousSet = "horizonExtra")
ggAlignPlots(p1[[1]], p1[[2]], type = "h")
plotPDF(p1, name = "trajectory1_NR2E3_inf_expression_UMAP.pdf", ArchRProj = proj1, addDOC = FALSE, width = 5, height = 5)
