...
##dave_pkd_modifier_0921
##homozygosity mapping
setwd('/home/atimms/ngs_data/enu_mapping/dave_pkd_modifier_0921')
##loop through files
filename <- dir('/home/atimms/ngs_data/enu_mapping/dave_pkd_modifier_0921', pattern ="combined_hom_mapping.txt")
for(i in 1:length(filename)){
  ##read in hom data
  hom_data <- read.csv(filename[i], sep ='\t')
  ##reorder chromosome, if using 'chr'
...
##test just graphing one at a time
hom_data <- read.csv('EP0014-0259-0009_mm10_10000kb_1000kb_combined_hom_mapping.txt', sep ='\t')
ggplot(data=hom_data,aes(x=chromosome, y=value),binwidth=1e5 ) + geom_bar(stat="identity", colour="black") + facet_grid(analysis ~ chr, scales="free", space="free_x") + 
  theme_bw() + theme(axis.text.x = element_blank())
ggsave('test.pdf', height = 4, width = 12)
...
##dave_xiaoer_1221
##homozygosity mapping
setwd('/home/atimms/ngs_data/enu_mapping/dave_xiaoer_1221')
##loop through files
filename <- dir('/home/atimms/ngs_data/enu_mapping/dave_xiaoer_1221', pattern ="combined_hom_mapping.txt")
for(i in 1:length(filename)){
  ##read in hom data
  hom_data <- read.csv(filename[i], sep ='\t')
  ##reorder chromosome, if using 'chr'
