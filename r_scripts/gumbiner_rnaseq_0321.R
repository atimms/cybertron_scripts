...
workingDir = "/home/atimms/ngs_data/rnaseq/gumbiner_rnaseq_0321";
setwd(workingDir);

###gumbiner_rnaseq_0321
##read in count and metadata
countData1 <- read.table('gumbiner_rnaseq_0321.star_fc.counts.txt', header=T, row.names=1)
colData1 <- read.table('gumbiner_rnaseq_0321.star_fc.metadata.txt', header=T, row.names=1)
head(countData1)
head(colData1)
...
write.csv(count_data, file="gumbiner_rnaseq_0321.norm_counts.csv")
...
#write.csv(assay(rld), file="gumbiner_rnaseq_0321.deseq.vst_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='gumbiner_rnaseq_0321.sample_heatmap.pdf', width = 7, height = 5)
##principal components analysis 
plotPCA(rld, intgroup = c("sample_name"))
ggsave('gumbiner_rnaseq_0321.sample_pca.pdf', width=6, height = 6)
plotPCA(rld, intgroup = c("condition"))
ggsave('gumbiner_rnaseq_0321.condition_pca.pdf', width=6, height = 6)
...
dev.copy2pdf(file='gumbiner_rnaseq_0321.25_var_gene_clustering.pdf', width = 10, height = 10)
...
write.csv(resOrdered2DF, file="gumbiner_rnaseq_0321.CTRL.treated_vs_untreated.csv")
