...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.qc.pdf", width=20)
...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.pca.pdf", width=20)
ElbowPlot(human, ndims=30)
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.elbow_plot.pdf", width=20)
...
saveRDS(human, file = "./seurat_analysis/adult/human.rds")
##and load if needed
#human <- readRDS(file = "./seurat_analysis/adult/human.rds")
...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.merged.UMAP.res0.4.sample_split.pdf", width = 20)
DimPlot(human_merged, reduction='umap', pt.size=0.1, label = TRUE)
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.merged.UMAP.res0.4.pdf", width=20)
...
write.csv(counts_cluster_sample, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.merged.counts_cluster_sample.res0.4.csv')
##graph known markers
...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.merged.res0.4.known_markers.dotplot.pdf", width = 20)
...
write.csv(human_merged.markers, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.merged.markers.res0.4.csv')
...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.harmony_plot.pdf", width = 20)
...
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.UMAP.res0.4.sample_split.pdf", width = 20)
DimPlot(human_harmony, reduction='umap', pt.size=0.1, label = TRUE)
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.UMAP.res0.4.pdf", width=20)
#Identify the number of cells in each cluster between samples
counts_cluster_sample = table(human_harmony$seurat_clusters, human_harmony$sample)
write.csv(counts_cluster_sample, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.counts_cluster_sample.res0.4.csv')
#Dot plot - the size of the dot = % of cells and color represents the average expression
DotPlot(human_harmony, features = markers) + RotatedAxis()
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.res0.4.known_markers.dotplot.pdf", width = 20)
#Find all markers that define each cluster
human_harmony.markers <- FindAllMarkers(human_harmony, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(human_harmony.markers, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.markers.res0.4.csv')

#Save object to avoid needing to re-run previous computations
saveRDS(human_harmony, file = "./seurat_analysis/adult/human_harmony.rds")


##add cluster names and then extract cells and cluster ids
human_harmony <- readRDS(file = "./seurat_analysis/adult/human_harmony.rds")

#Add celltype info
##make marked umap
new.cluster.ids <- c('Rods', 'Rods with MT reads', 'Mulllers', 'Bipolars', 'Cones', 'Bipolars', 'Amacrines', 'Bipolars', 'Amacrines', 'Amacrines', 'Horizontals', 'Bipolars', 'Bipolars', 'Bipolars', 'Bipolars', 'Bipolars', 'Ganglions', 'Astrocytes', 'Amacrines', 'Microglia', 'Bipolars', 'Ganglions', 'Amacrines', 'Bipolars', 'Amacrines', 'Amacrines', 'Amacrines')
names(new.cluster.ids) <- levels(human_harmony)
human_harmony <- RenameIdents(human_harmony, new.cluster.ids)
DimPlot(human_harmony, reduction = "umap", pt.size = 0.1, label = TRUE)
dev.copy2pdf(file="./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.UMAP_celltypes.res0.4.pdf", width=10, height=10)

##get barcode info for each cluster
WhichCells(object = human_harmony, idents = c('Rods', 'Rods with MT reads', 'Mulllers', 'Bipolars', 'Cones', 'Amacrines', 'Horizontals', 'Ganglions', 'Astrocytes', 'Microglia'))
human_harmony$celltype <- plyr::mapvalues(
  ...
  to = c('Rods', 'Rods with MT reads', 'Mulllers', 'Bipolars', 'Cones', 'Bipolars', 'Amacrines', 'Bipolars', 'Amacrines', 'Amacrines', 'Horizontals', 'Bipolars', 'Bipolars', 'Bipolars', 'Bipolars', 'Bipolars', 'Ganglions', 'Astrocytes', 'Amacrines', 'Microglia', 'Bipolars', 'Ganglions', 'Amacrines', 'Bipolars', 'Amacrines', 'Amacrines', 'Amacrines')
)
head(human_harmony$celltype)
colnames(human_harmony)
cell.info = human_harmony$celltype
write.csv(cell.info, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.res0.4.cellinfo.csv')

##get counts per sample/cell class
sample.celltype.info = table(human_harmony$celltype, human_harmony$orig.ident)
write.csv(sample.celltype.info, file='./seurat_analysis/adult/human_scrnaseq_adult_0620.harmony.res0.4.sample_cell_info.csv')
