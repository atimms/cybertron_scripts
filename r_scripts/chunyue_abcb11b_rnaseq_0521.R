...
workingDir = "/home/atimms/ngs_data/rnaseq/chunyue_abcb11b_rnaseq_0521";
setwd(workingDir);

###all samples
##read in count and metadata
countData1 <- read.table('chunyue_abcb11b_0521.star_fc.counts.txt', header=T, row.names=1)
colData1 <- read.table('chunyue_abcb11b_0521.star_fc.metadata.txt', header=T, row.names=1)
head(countData1)
head(colData1)
...
write.csv(count_data, file="chunyue_abcb11b_0521.norm_counts.csv")
...
#write.csv(assay(rld), file="chunyue_abcb11b_0521.deseq.vst_counts.csv")
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
dev.copy2pdf(file='chunyue_abcb11b_0521.sample_heatmap.pdf', width = 7, height = 5)
##principal components analysis 
plotPCA(rld, intgroup = c("sample_name"))
ggsave('chunyue_abcb11b_0521.sample_pca.pdf', width=6, height = 6)
plotPCA(rld, intgroup = c("genotype"))
ggsave('chunyue_abcb11b_0521.condition_pca.pdf', width=6, height = 6)
...
dev.copy2pdf(file='chunyue_abcb11b_0521.25_var_gene_clustering.pdf', width = 10, height = 10)
...
res2 <- results(dds, contrast=c("genotype", "RM", "WT"))
##get summary
...
resOrdered2DF <- as.data.frame(resOrdered2)[1:20453,]
write.csv(resOrdered2DF, file="chunyue_abcb11b_0521.RM_vs_WT.csv")
