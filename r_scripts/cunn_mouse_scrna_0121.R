...
setwd('/home/atimms/ngs_data/misc/cunn_mouse_scrna_0121')
...
mouse_data = Read10X(data.dir = 'filtered_feature_bc_matrix_intronic')
seurat_mat <- CreateSeuratObject(counts = mouse_data, project = "mouse_sutures", min.cells = 3, min.features = 200)
seurat_mat
...
seurat_mat <- subset(seurat_mat, subset = nFeature_RNA > 200 & nFeature_RNA < 7000 & percent.mt < 10)
...
dev.copy2pdf(file="mouse_sutures_0121.umap.pdf", width = 10, height = 10)
...
##save file
saveRDS(seurat_mat, file = "mouse_sutures_0121.rds")

##and then load
seurat_mat <- readRDS(file ="mouse_sutures_0121.rds")
