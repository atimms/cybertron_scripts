...
previous_exomes_data = ref_dir + 'gemini/all_exome_data_std_pipeline.0317.xls'
...
def load_single_vcf_into_gemini(ped, db_name, input_vcf):
	...
	gemini_load = subprocess.Popen([gemini, 'load', '--cores', '20', '-t', 'snpEff', '-p', ped_file, '--tempdir', './temp',  '-v', normalized_vcf + '.gz', db_name])
	gemini_load.wait()
