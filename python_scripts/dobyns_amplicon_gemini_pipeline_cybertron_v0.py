...
def combine_gemini_results(ped, file_list, gene_col, position_to_insert, position_to_finish, add_annovar_pos, out_suffix, norm_vcf):
	##file names
	...
	##add dbdb and mgi data if any variants
	if var_count >= 1:
		...
		add_annovar_ts_info(ped, temp2_file, outfile, add_annovar_pos, norm_vcf)
...
def format_autosomal_dominant_analysis(ped, file_list, gene_col, position_to_insert, position_to_finish, add_annovar_pos, out_suffix, norm_vcf):
	##file names
	outfile = ped + out_suffix
	temp_file = 'combined.temp.xls'
	temp2_file = 'combined.temp2.xls'
...
def standard_gemini_protocol(working_directory, pedigree, ped_type):
	...
	##this does all three
	# analysis_dict = {'in_both':['.in_both.gemini.db', '.in_both.gemini_ad.db', '.intersected_vcfs/0002.vcf', '.in_both.std_analysis.xls', '.in_both.int.norm.vcf','.in_both.auto_dom_analysis.xls'],
	# 			'gatk':['.gatk.gemini.db', '.gatk.gemini_ad.db', '.intersected_vcfs/0000.vcf', '.gatk_only.std_analysis.xls', '.gatk.int.norm.vcf','.gatk.auto_dom_analysis.xls'],
	# 			'fb':['.fb.gemini.db', '.fb.gemini_ad.db', '.intersected_vcfs/0001.vcf', '.fb_only.std_analysis.xls', '.fb.int.norm.vcf','.fb.auto_dom_analysis.xls']}
	# analysis_dict = {'gatk':['.gatk.gemini.db', '.gatk.gemini_ad.db', '.gatk.vcf.gz', '.gatk.std_analysis.xls', '.gatk.int.norm.vcf','.gatk.auto_dom_analysis.xls'],
	# 			'fb':['.fb.gemini.db', '.fb.gemini_ad.db', '.freebayes.vcf.gz', '.fb.std_analysis.xls', '.fb.int.norm.vcf','.fb.auto_dom_analysis.xls']}
	analysis_dict = {'in_both':['.in_both.gemini.db', '.in_both.gemini_ad.db', '.intersected_vcfs/0002.vcf', '.in_both.std_analysis.xls', '.in_both.int.norm.vcf','.in_both.auto_dom_analysis.xls']}
	for analysis in analysis_dict:
		...
		ad_analysis_file_suffix = analysis_dict[analysis][5]
		...
		if ped_type == 'singleton':
			...
			format_autosomal_dominant_analysis(pedigree, [auto_dom_file], 5, 13, 22, 13, ad_analysis_file_suffix, norm_vcf)
		else:
			print 'ped type %s not recognized'%ped_type
		##combine results
		combine_gemini_results(pedigree, files_to_combine, 5, 13, 22, 6, analysis_file_suffix, norm_vcf)
		# '''
