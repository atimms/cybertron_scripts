	...
	for sample in samples:
		...
		with open(multianno, "r") as av, open(outfile, "w") as final:
			...
			for line in av:
				...
				line = line.strip('\n').split(delim)
				if lc == 1:
					header = line[:16] + ['TP1FRes','TP1FUnr', 'zygosity', 'qual', 'cov', 'info', 'format', 'vcf']
					final.write(delim.join(header) + '\n')
				else:
					line_out = line[:21] + line[28:] 
...
def make_snptrack_score(mut_name, control_name, window_sizes):
	bed_dict = {}
	mut_het_bg = mut_name + '_' + window_sizes + '_het_count.bedgraph'
	ctl_het_bg = control_name + '_' + window_sizes + '_het_count.bedgraph'
	mut_hom_bg = mut_name + '_' + window_sizes + '_hom_count.bedgraph'
	##use 'informative' hom counts
	mut_hom_bed = mut_name + '.resc_hom.' + window_sizes + '.bed'
	mut_hom_bed_no_head = mut_name + '.resc_hom.' + window_sizes + '_temp.bed'
	out_bed = mut_name + '_' + window_sizes + '.snptrack_hom.bed'
	##remove header of hom count file and add chr
	with open(mut_hom_bed_no_head, "w") as out_fh, open(mut_hom_bed, "r") as in_fh:
		lc = 0
		for line in in_fh:
			lc += 1
			if lc > 1:
				##addding the chr from start of the line
				out_fh.write('chr' + line)	

	for infile in [ctl_het_bg, mut_het_bg, mut_hom_bg, mut_hom_bed_no_head]:
		with open(infile, "r") as in_fh:
			for line in in_fh:
				...
				count = line[3]
				if cse in bed_dict:
					bed_dict[cse].append(count)
				else:
					bed_dict[cse] = [count]
	with open('temp.bed', "w") as temp_fh:
		for pos in bed_dict:
			print(pos, bed_dict[pos])
			ctl_het = float(bed_dict[pos][0])
			mut_het = float(bed_dict[pos][1])
			mut_hom = float(bed_dict[pos][2])
			mut_hom_alt = float(bed_dict[pos][3])
			##remove regions that have 0 het counts in both pools
			if ctl_het + mut_het > 0:
				hom_score1 = (ctl_het/(ctl_het+mut_het)) * mut_hom
				hom_score2 = (ctl_het/(ctl_het+mut_het)) * mut_hom_alt
				# line_out = delim.join(pos.split('_') + [str(hom_score), str(ctl_het), str(mut_het), str(mut_hom)])
				line_out = delim.join(pos.split('_') + [str(hom_score1), str(hom_score2)])
				temp_fh.write(line_out + '\n')
	##sort and add header
	with open('temp2.bed', "w") as t2_fh:
		sort_file = subprocess.Popen(['sort', '-k1,1', '-k2,2n', 'temp.bed'], stdout=t2_fh)
		sort_file.wait()	
		##add header
		with open(out_bed, "w") as out_fh, open('temp2.bed', "r") as in_fh:
			out_fh.write(delim.join(['chr', 'start', 'end', 'hom_score_all', 'hom_score_rescued']) + '\n')
			for line in in_fh:
...
##run methods
work_dir = '/home/atimms/ngs_data/enu_mapping/chunyue_abcb11b_recessive_0321'
os.chdir(work_dir)


##params
project_name = 'chunyue_abcb11b_recessive_0321'
...
for ws in window_size:
	for sample in samples:
		##remove if in rmsk
		filtering_annotated.filter(working_dir, "and", sample + '.annotated.xls', sample + "11.temp", [16], ['=='], [''])

		##remove indels
		filtering_annotated.filter(working_dir, "and", sample + "11.temp", sample + "12.temp", [4,5], ['!=','!='], ['-','-'])

		##filter variants by coverage and quality 
		filtering_annotated.filter(working_dir, "and", sample + "12.temp", sample + '.hom_temp.txt', [cov_col,qual_col], ['>=','>='], [cov_definition,qual_definition])
...
##map counts of snps hom in rescued and het/wt in unrescued
...
for sample in samples:
	...
	##keep if not hom in unrescued
	filtering_annotated.filter(working_dir, "and", sample + "24.temp", sample + hom_snp_suffix, [18], ['!='], ['hom'])
...
##lets replicate the snptrack homozygosity score i.e.
##ratio of heterozygous SNP calls between control and mutant pools multiplied by the number of informative homozygous SNP calls in the mutant pool
rescued_name = 'TP1FRes'
unrescued_name = 'TP1FUnr'
size_names = ['danRer11_100kb_10kb', 'danRer11_1000kb_100kb']
