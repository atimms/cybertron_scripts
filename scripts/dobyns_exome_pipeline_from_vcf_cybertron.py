...
def load_single_vcf_into_gemini(ped, db_name, input_vcf, ped_file):
	...
	gemini_load = subprocess.Popen([gemini, 'load', '--cores', '10', '-t', 'snpEff', '-p', ped_file, '--tempdir', gemini_temp_dir,  '-v', normalized_vcf + '.gz', db_name])
	gemini_load.wait()
...
def gemini_potential_cnv(ped, gem_db_name, max_aaf, coverage):
	...
	all_auto_rec_file = ped + '.all_auto_rec.temp.xls'
	correct_auto_rec_file = ped + '.correct_auto_rec.temp.xls'
...
def combine_gemini_results(ped, file_list, gene_col, position_to_insert, position_to_finish, add_annovar_pos, add_aaf_pos):
	##file names
	...
	add_aaf_info(ped, temp3_file, outfile, add_aaf_pos)
	##remove intermediate files
	os.remove(temp_file)
...
def prepare_vcf_for_gemini(vcf_dict, final_vcf):
	all_vcfs = []
	for sample in vcf_dict:
		in_vcf = vcf_dict[sample]
		renamed_vcf = sample + '.ind.vcf.gz'
		##index vcf
		tabix_vcf = subprocess.Popen(['tabix', '-p', 'vcf', in_vcf])
		tabix_vcf.wait()
		##rename sample in the vcf 
		picard_rsv = subprocess.Popen([picard, 'RenameSampleInVcf', 'INPUT=' + in_vcf, 'O=' + renamed_vcf, 'NEW_SAMPLE_NAME=' + sample])
		picard_rsv.wait()
		bcf_index = subprocess.Popen([bcftools_12, 'index', renamed_vcf])
		bcf_index.wait()
		all_vcfs.append(renamed_vcf)
	##combine vcfs
	bcf_merge = subprocess.Popen([bcftools_12, 'merge'] + all_vcfs + ['-O', 'z', '-o', final_vcf, '-m', 'none'])
	bcf_merge.wait()
...
def standard_gemini_protocol(working_directory, pedigree, ped_type, sample_vcf_dict):
	os.chdir(working_directory)
	gemini_db = pedigree + '.gemini.db'
	ad_db = pedigree + '.gemini_ad.db'
	ped_vcf = pedigree + '.all_samples.vcf.gz'
	...
	coverage_req = 10
	##prep and merge vcfs
	# prepare_vcf_for_gemini(sample_vcf_dict, ped_vcf)
	##load vcf file into db
	# '''
	load_single_vcf_into_gemini(pedigree, gemini_db, ped_vcf, std_ped_file)
	...
	##combine results
	genes_in_file = combine_gemini_results(pedigree, files_to_combine, 5, 13, 22, 13, 31)
...
##ghayday 0618 one ped from vcf
working_dir = '/home/atimms/ngs_data/exomes/working/ghayda_0618'
vcf_dict = {'LR18-063': '1534805.vcf.gz', 'LR18-063f': '1534806.vcf.gz', 'LR18-063m': '1537560.vcf.gz'}
standard_gemini_protocol(working_dir, "LR18-063", 'singleton', vcf_dict)
