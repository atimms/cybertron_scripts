...
def load_single_vcf_into_gemini(ped, db_name, input_vcf):
	...
	temp_vcf = 'temp1.vcf'
	normalized_vcf = ped + '.int.norm.inh.vcf'
	ped_file = ped + '.inh.ped'
...
def inherited_gemini_protocol(working_directory, pedigree, ped_type):
	os.chdir(working_directory)
	gemini_db = pedigree + '.gemini.inh.db'
	...
	files_to_combine = []
	if ped_type == 'trio':
		files_to_combine.append(gemini_dominant(pedigree, gemini_db, freq_req_recessive, coverage_req))

	else:
		print 'ped type %s not recognized'%ped_type
	##combine results
	genes_in_file = combine_gemini_results(pedigree, files_to_combine, 5, 13, 22, 6, 24)
	##get all variants seen 
	# get_previous_vars(genes_in_file, pedigree + '.previous_vars_in_same_genes.xls')

working_dir = '/home/atimms/ngs_data/exomes/working/ghayda_0418/b4'
prefix = 'LR17-106'
ped_type = 'trio'
inherited_gemini_protocol(working_dir, prefix, ped_type)
