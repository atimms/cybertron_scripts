...
delim = '\t'
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219'
...
def get_vcf_per_ped(pedigrees, in_vcf, work_dir):
	os.chdir(work_dir)
...
def combine_results_files(peds_to_analyze, work_dir):
	...
	combine_files(trio_peds, combined_trio_dom, '.cov10.maf0.01.autosomal_dominant.xls')
	combine_files(single_duo_peds, combined_duo_single, '.std_analysis.xls')

def make_unaffected_vcf(sample_file, in_vcf, out_vcf, work_dir):
	os.chdir(work_dir)
	temp_vcf = 'temp_unaffected.vcf.gz'
	bcftools_view = subprocess.Popen([bcftools, 'view', '-a', '--threads', '10', '--force-samples', '-Ou', '-S', sample_file, in_vcf], stdout=subprocess.PIPE)
	bcftools_view2 = subprocess.Popen([bcftools, 'view', '-m', '2', '--threads', '10', '-O', 'z', '-o', temp_vcf, '-'], stdin=bcftools_view.stdout)
	bcftools_view2.wait()

	##normalize vcf with vt
	with open (out_vcf, 'w') as ovcf_fh:
		zless_vcf = subprocess.Popen(['zless', temp_vcf], stdout=subprocess.PIPE)
		sed_vcf = subprocess.Popen(['sed', 's/ID=AD,Number=./ID=AD,Number=R/'], stdin=zless_vcf.stdout, stdout=subprocess.PIPE)
		vt_decompose = subprocess.Popen(['vt', 'decompose', '-s', '-'], stdin=sed_vcf.stdout, stdout=subprocess.PIPE)
		vt_normalize = subprocess.Popen(['vt', 'normalize', '-r', fasta, '-'], stdin=vt_decompose.stdout, stdout=ovcf_fh)
		vt_normalize.wait()
		bgzip_vcf = subprocess.Popen(['bgzip', out_vcf])
		bgzip_vcf.wait()
		tabix_vcf = subprocess.Popen(['tabix', '-p', 'vcf', out_vcf + '.gz'])
...
##run methods
##tidy reference files
##needd to add 'chr' to contig names i.e. gunzip -c clinvar_20190211.vcf.gz | awk '{if($0 !~ /^#/) print "chr"$0; else print $0}' > clinvar_20190211.added_chr.vcf
##and then tabix i.e. tabix -p vcf gnomad.exomes.r2.0.1.sites.GRCh38.noVEP.added_chr.vcf.gz
'''
not needed !!!!
# ref_vcfs = ['ExAC.0.3.GRCh38.added_chr.vcf.gz', 'gnomad.exomes.r2.0.1.sites.GRCh38.noVEP.added_chr.vcf.gz', 'clinvar_20190211.added_chr.vcf.gz']
# ref_vcfs = ['clinvar_20190211.added_chr.vcf.gz']
##run vt on the file, needed?
# vt_tidy_vcf(ref_dir, ref_vcfs)
##to annotate modify files downloaded from github
...
original_vcfs = ['GMKF_Gabriel_Luquetti_Craniofacial_WGS_v2.vcf.gz', 'GMKF_Gabriel_Luquetti_Craniofacial_WGS_PCR_Plus_v2.vcf.gz']
merged_vcf = 'GMKF_Luquetti_Craniofacial_WGS_passed.vcf.gz'
unaffected_file = 'unaffected.txt'
unaffected_vcf = 'GMKF_Luquetti_Craniofacial_WGS_passed_unaffected.vcf'
# peds_to_analyze = ['CFM-MOS-12']
...
peds_to_analyze_1 = ['CFM-MOS-12', 'PM1060018', 'CL3003', 'CL3023', 'CL3051', 'F040', 'CFM-MOS-11', 'F168', 'CL1003', 'CL1016', 
		'CL1020', 'CL3033', 'CL3044', 'CL3045', 'CL4001', 'CL4004', 'CL4005', 'CL4007', 'F003', 'F005', 'F007', 'F008', 'F009']
peds_to_analyze_2 =	['F011', 'F015', 'F016', 'F017', 'F020', 'F0200003', 'F0200013', 'F0200022', 'F022', 'F027', 'F028', 'F0400012', 'F0400020', 
		'F041', 'F042', 'F045', 'F047', 'F050', 'F093', 'F149', 'F150', 'F152', 'F154', 'F200006', 'F300031', 'F300059', 'F300073']
peds_to_analyze_3 =	['F300097', 'F300111', 'F30028', 'F400017', 'F400025', 'PM1010002', 'PM1010004', 'PM1010005', 'PM1010007', 'PM1010010', 'PM1010017', 
		'PM1010036', 'PM1010037', 'PM1010039', 'PM1010040', 'PM1010042', 'PM1010043', 'PM1010044', 'PM1010045', 'PM1030019', 'PM1030021']
peds_to_analyze_4 = ['PM1030022', 'PM1030025', 'PM1030028', 'PM1030029', 'PM1030030', 'PM1030031', 'PM1060019', 'PM1060028', 'PM1060029', 'PM1060030', 
		'PM1060031', 'PM1060032', 'PM1070005', 'PM1070017', 'PM1070019', 'PM1070020', 'PM4010001', 'PM4010003', 'PM4010004', 'PM4010005']
peds_to_reanalyze = ['CL1003', 'CL1016', 'CL1020', 'CL3003', 'CL3023', 'CL3033', 'CL3044', 'CL3045', 'CL3051', 'CL4001', 'CL4004', 'CL4005', 'CL4007', 'PM1030029', 'PM1010040', 'PM1010037', 'PM1030022']

##for catch up...
peds_to_analyze_2 =	['F300031', 'F300059', 'F300073']
peds_to_analyze_3 =	['PM1030019', 'PM1030021']
peds_to_analyze_4 = ['PM4010004', 'PM4010005']
peds_to_analyze_4 = ['PM1060029']
peds_to_reanalyze_1 = ['CL1020', 'CL3003', 'CL3023', 'CL3033', 'CL3044']
peds_to_reanalyze_2 = ['CL3045', 'CL3051', 'CL4001', 'CL4004', 'CL4005']
peds_to_reanalyze_3 = ['CL4007', 'PM1030029', 'PM1010040', 'PM1010037', 'PM1030022']


##make 1 vcf per pedigree
##merge into single vcf
# merge_vcf_files(original_vcfs, merged_vcf)

##extract sample per ped, bcftools view to get one vcf per ped
##working directory
# working_dir = '/home/atimms/ngs_data/genomes/daniela_0219'
# get_vcf_per_ped(peds_to_analyze, merged_vcf,working_dir)
# gemini_protocol(peds_to_analyze_1, working_dir)

##split 4 ways
##get samples from vcf, annotate, load into gemnini and then query
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/b1'
# get_vcf_per_ped(peds_to_analyze_1, merged_vcf, working_dir)
# gemini_protocol(peds_to_analyze_1, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/b2'
# get_vcf_per_ped(peds_to_analyze_2, merged_vcf, working_dir)
# gemini_protocol(peds_to_analyze_2, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/b3'
# get_vcf_per_ped(peds_to_analyze_3, merged_vcf, working_dir)
# gemini_protocol(peds_to_analyze_3, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/b4'
# get_vcf_per_ped(peds_to_analyze_4, merged_vcf, working_dir)
# gemini_protocol(peds_to_analyze_4, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/repeats'
# gemini_protocol(peds_to_reanalyze, working_dir)
# gemini_protocol(peds_to_reanalyze_1, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/r2'
# gemini_protocol(peds_to_reanalyze_2, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/r3'
# gemini_protocol(peds_to_reanalyze_3, working_dir)

##if ped includes samples not in vcf need to redo (should change code in future)
#/home/atimms/programs/bcftools-1.9/bcftools view -a -Ou -S PM1060029.samples_temp.txt GMKF_Luquetti_Craniofacial_WGS_passed.vcf.gz | /home/atimms/programs/bcftools-1.9/bcftools view -m 2 -O z -o PM1060029.vcf.gz

##check relatedness
'''
##first time
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/r_check'
for ped in peds_to_analyze:
	vcf_file = ped + '.vcf.gz'
	plink_relatadness_check(vcf_file, ped, working_dir)

##on repaets
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/r_check2'
for ped in peds_to_reanalyze:
	vcf_file = ped + '.vcf.gz'
	plink_relatadness_check(vcf_file, ped, working_dir)
'''

##combined files, three ways all trios (std), all trios (auto dom), and duos/singeltons
# peds_to_analyze = ['F168', 'F040', 'F003', 'F005']
# working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/comb_test'
# combine_results_files(peds_to_analyze, working_dir)
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219'
# combine_results_files(peds_to_analyze, working_dir)
##add parental data
make_unaffected_vcf(unaffected_file, merged_vcf, unaffected_vcf, working_dir)

##issue with annotation --- test
'''
working_dir = '/home/atimms/ngs_data/genomes/daniela_0219/test'
peds_to_test = ['CL1020', 'CL4005']
peds_to_test = ['CL4005']
gemini_protocol(peds_to_test, working_dir)
'''
