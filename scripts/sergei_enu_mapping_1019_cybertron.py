...
##working dir
working_dir = '/home/atimms/ngs_data/enu_mapping/sergei_enu_1019'
...
av_protocol = ['-protocol', 'refGene,rmsk,genomicSuperDups,snp142,generic,generic,generic,generic,generic,generic,generic,generic,generic,generic,generic,generic,generic,generic', '-genericdbfile', 'mgp.v5.merged.snps_all.dbSNP142.avinput,C3H_HeH.mgp.v5.snps.dbSNP142.avinput,C3H_HeJ.mgp.v5.snps.dbSNP142.avinput,combined_mutant.avinput,M392.avinput,M398.avinput,M409.avinput,M558.avinput,1493.avinput,1524.avinput,1525.avinput,1526.avinput,C3H.avinput,mut_combined.avinput']
...
def multianno_to_annotated(samples): 
	head = ['Chr', 'Start', 'End', 'Ref', 'Alt', 'Func.refGene', 'Gene.refGene', 'GeneDetail.refGene', 'ExonicFunc.refGene', 'AAChange.refGene', 'rmsk', 'genomicSuperDups', 'snp142','mgp.v5.snps', 'C3H_HeH', 'C3H_HeJ', 'combined_mutant', 'M392', 'M398', 'M409', 'M558', '1493', '1524', '1525', '1526', 'C3H', 'mut_combined', 'Zygosity', 'Qual', 'Coverage', 'Chr', 'Pos', 'Filter', 'Ref2', 'Alt2','Qual', 'Filter', 'GT_info', 'Format', 'Info']
...
fq_dict = {'1493':['1493_1.fq.gz', '1493_2.fq.gz'], '1524':['1524_1.fq.gz', '1524_2.fq.gz'],
		'1525':['1525_1.fq.gz', '1525_2.fq.gz'], '1526':['1526_1.fq.gz', '1526_2.fq.gz'],
		'C3H':['C3H_1.fq.gz', 'C3H_2.fq.gz']}
...
##temp for catchup
# variant_calling_samtools(['C3H'], mkdup_bam, st_vcf_suffix)
convert_to_annovar(fq_dict, st_vcf_suffix + '.gz')
run_table_annovar(fq_dict)
multianno_to_annotated(fq_dict)
'''

##combine mutatnt fqs and call vars etc
samples = fq_dict.keys() + ['mut_combined']
# '''
##combine the three mutatnt fq files
mutant_r1_fqs = ['1524_1.fq.gz', '1525_1.fq.gz', '1526_1.fq.gz']
mutant_r2_fqs = ['1524_2.fq.gz', '1525_2.fq.gz', '1526_2.fq.gz']
combined_mut_r1_fq = 'comb_mut_1.fq.gz'
combined_mut_r2_fq = 'comb_mut_2.fq.gz'
combined_fq_dict = {'mut_combined':[combined_mut_r1_fq, combined_mut_r2_fq]}


# combine_fq_file(mutant_r1_fqs, mutant_r2_fqs, combined_mut_r1_fq, combined_mut_r2_fq)
# align_with_bwa(combined_fq_dict)
# variant_calling_samtools(combined_fq_dict, mkdup_bam, st_vcf_suffix)
# convert_to_annovar(combined_fq_dict, st_vcf_suffix + '.gz')
# run_table_annovar(combined_fq_dict)
# multianno_to_annotated(combined_fq_dict)
# '''
...
##using those 'in' dbsnp for just mut_combined
# samples = ['mut_combined']
# for ws in window_size:
# 	for sample in samples:
# 		##remove if in rmsk, segdup
# 		filtering_annotated.filter(working_dir, "and", sample + '.annotated.txt', sample + "21.temp", [11,12], ['==','=='], ['',''])
# 		filtering_annotated.filter(working_dir, "and", sample + '21.temp', sample + "31.temp", [13], ['!='], [''])
# 		##filter variants by coverage and quality 
# 		filtering_annotated.filter(working_dir, "and", sample + "31.temp", sample + '.hom_temp.txt', [cov_col,qual_col], ['>=','>='], [cov_definition,qual_definition])
...
##using those in the C3H mouse for just mut_combined

'''
samples = ['mut_combined']
for ws in window_size:
	for sample in samples:
		##remove if in rmsk, segdup and keep if in c3h mouse
		filtering_annotated.filter(working_dir, "and", sample + '.annotated.txt', sample + "21.temp", [11,12], ['==','=='], ['',''])
		filtering_annotated.filter(working_dir, "and", sample + '21.temp', sample + "31.temp", [26], ['=='], ['hom'])
		##filter variants by coverage and quality 
		filtering_annotated.filter(working_dir, "and", sample + "31.temp", sample + '.hom_temp.txt', [cov_col,qual_col], ['>=','>='], [cov_definition,qual_definition])
		##get hom vars
		filtering_annotated.filter(working_dir, "and", sample + ".hom_temp.txt", sample + '.no_rpts_c3h_hom.xls', [zygosity_col], ['=='], ['hom'])
...
for ws in window_size:
	for sample in samples:
		...
		filtering_annotated.filter(working_dir, "and", sample + '.annotated.txt', sample + "21.temp", [11,12], ['==','=='], ['',''])
		filtering_annotated.filter(working_dir, "and", sample + '21.temp', sample + "31.temp", [22], ['!='], ['hom'])
		filtering_annotated.filter(working_dir, "and", sample + '31.temp', sample + "41.temp", [26], ['=='], ['hom'])
		##filter variants by coverage and quality 
		filtering_annotated.filter(working_dir, "and", sample + "41.temp", sample + '.hom_temp.txt', [cov_col,qual_col], ['>=','>='], [cov_definition,qual_definition])
		##get hom vars
		filtering_annotated.filter(working_dir, "and", sample + ".hom_temp.txt", sample + '.no_rpts_c3h_notunaff_hom.xls', [zygosity_col], ['=='], ['hom'])
...
bedgraphs_to_combine = ['1524_mm10_10000kb_100kb_naf.bedgraph', '1525_mm10_10000kb_100kb_naf.bedgraph', '1526_mm10_10000kb_100kb_naf.bedgraph']
mut_bedgraph = '1493_mm10_10000kb_100kb_naf.bedgraph'
